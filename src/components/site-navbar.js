...
class SiteNavbar extends HTMLElement {
  ...
  renderAuthControls() {
    ...
    authControls.innerHTML = `<div class="btn btn-outline-light text-dark" style="visibility: hidden; min-width: 80px;">Log out</div>`;

    onAuthStateChanged(auth, (user) => {
      let updatedAuthControl;
      if (user) {
        updatedAuthControl = `<button class="btn btn-outline-light text-dark" id="signOutBtn" type="button" style="min-width: 80px;">Log out</button>`;
